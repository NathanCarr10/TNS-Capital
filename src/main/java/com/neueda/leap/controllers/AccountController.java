...
public class AccountController {
    ...
    public ResponseEntity<BalanceResponse> getAccountBalance(@PathVariable Long accountId) {
        // Validates account exists before returning balance; prevents exposing non-existent accounts
        Account account = accountRepository.findById(accountId)
                .orElseThrow(() -> new AccountNotFoundException("Account not found: " + accountId));
    ...
    public ResponseEntity<List<PositionResponse>> getAccountPositions(@PathVariable Long accountId) {
        // Validates account exists; queries positions separately to enable flexible retrieval
    ...
    public ResponseEntity<List<OrderResponse>> getAccountOrders(@PathVariable Long accountId) {
        // Validates account exists; queries orders separately to enable filtering and pagination later
        accountRepository.findById(accountId)
                .orElseThrow(() -> new AccountNotFoundException("Account not found: " + accountId));
    ...
    private PositionResponse mapPositionToResponse(Position position) {
        // Maps Position entity to response; uses zero marketValue since real-time pricing unavailable in this context
    ...
    private OrderResponse mapOrderToResponse(Order order) {
        ...
        return new OrderResponse(
                order.getId().getLeastSignificantBits(),  // Extracts Long from UUID's least significant 64 bits
                order.getAccountId(),
                order.getSymbol(),
                order.getSide(),
                order.getQuantity(),
                order.getPrice(),
                order.getStatus(),
                java.time.LocalDateTime.ofInstant(order.getCreatedOn(), java.time.ZoneId.of("UTC"))
        );
    }
